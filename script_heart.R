...
pairs(data, lower.panel = lower.panel, upper.panel = upper.panel) 
# This is long to execute, wait for it.
...
# AIC = 176.28 | ACCURACY = 96% | PRECISION = 97% | Recall = 94%
#-----------------------------------------
...
mod.gam = glm(HeartDisease ~ ns(Age, knots=c(47,54,60)) + Sex + ChestPainType + 
                ...
                RestingECG + ns(MaxHR, knots=c(119,137,155)) + ExerciseAngina +
...
evaluate.gam(mod.gam) #AIC = 178.76 | Precision = 97% | Recall = 96%
...
evaluate.gam(mod.gam3) #AIC = 166.96. That's the best model so far in terms of AIC.
                       #Accuracy = 95%, Precision = 95%, Recall = 96%
...
# 6.1. COMPARISON OF AICs --------------------------------------------------

#install.packages("AICcmodavg")
library(AICcmodavg)

all_models = list("mod.1"=mod.1, "model_10"=model_10,"model_11"=model_11, 
                  "model_12"=model_12,"mod.gam"=mod.gam, "mod.gam2"=mod.gam2, 
                  "mod.gam3"=mod.gam3)
models_names = c("Intuition", "model 10 var", "model 11 var", "model 12 var",
                 "mod.gam (Natural Spline)", "mod.gam2 (Natural Spline)", 
                 "mod.gam3 (Smoothing Spline)")
...
# However, model_11 has less parameters to estimate, and still
# have an accuracy = 96%, precision = 97%, recall = 94%.


# 6.2. Comparison of cross-validation errors and AIC -----------------------

library(boot)

cv_table = data.frame("model" = as.character(), "cv score" = as.numeric(),
                      "AIC" = as.numeric)
...
# lowest AIC. It seems to be mod.gam3 or model_11. Therefore, we can
# ask ourselves if the two models are significantly different or if using
# model_11 could be enough.
...
# 7.1. Are model_11 and mod.gam3 SIGNIFICANTLY DIFFERENT ?
anova(model_11, mod.gam3, test="Chisq")

# Conclusion: Yes. The 2 models are significantly different.


# 7.2. WHICH VARIABLE ARE THE MOST IMPORTANT ? -----------------------------
...
rank_11
# For model_11, ChestPainType (especially when it is NAP or ATA) is the most
# important variable to consider in order to predict Heart failure. The next
# most important variables are SexM, Cholesterol and type 1 ExerciseAngina.
...
# an individual can also matter for a better prediction.
...
#            data = train)
#
# AIC = 166.96 | Accuracy = 95% | Precision = 95% | Recall = 96%
...
# 10 compared to a classical logistic regression (see Delta_AICc on the aictab).
# Building mod.gam3 requires 18 parameters (see K on the aictab), 
# whereas model_11 (AIC = 176 | Accuracy = 96%) requires only 12 parameters and
# gets results quite similar with an even better precision (97%) but a smaller
# recall (94%): 
# 3% of the predicted Healthy had indeed a Heart Disease.
# 6% of the predicted Heart Disease were indeed Healthy.

# But, we are mostly interested in reducing false negative for rare medecine 
# data modelling. Therefore, the model using GAM is the best model.
#---------------------------------------------------------------------
